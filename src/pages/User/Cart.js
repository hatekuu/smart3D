...
const Cart = () => {
  ...
  const [isUpdating, setIsUpdating] = useState(false); // Để theo dõi trạng thái khi đang update
  const [debounceTimer, setDebounceTimer] = useState(null); // Để lưu trữ timer cho debounce
  ...
  // Hàm để xử lý khi muốn cập nhật quantity của sản phẩm với debounce
  const handleUpdateQuantity = (productId, quantity) => {
    // Nếu có timer trước đó, hủy bỏ nó
    if (debounceTimer) {
      clearTimeout(debounceTimer);
    }

    // Thiết lập timer mới để chờ đợi thời gian debounce
